...
class PurchasePointPayment {
    ...
    PurchasePointPayment(final Customer customer, final Comic comic) {
        if (customer.isDisabled()) throw new IllegalArgumentException("有効な購入者ではありません。");
        if (comic.isDisabled()) throw new IllegalArgumentException("現在取り扱いのできないコミックです。");
        if (customer.isShortOfPoint(comic)) throw new RuntimeException("所持ポイントが不足しています。");
    ...
    public String toString() {
        return customerId + ", " + comicId + ", " + consumptionPoint + ", " + paymentDateTime;
    }
}
